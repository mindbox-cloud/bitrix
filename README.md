...
**Точка интеграции** - Уникальный идентификатор сайта/мобильного приложения/и т.п. Значение нужно уточнить у менеджера.\
**Секретный ключ** - Каждой "Точка интеграции" соответствует свой "Секретный ключ", значение нужно уточнить у менеджера.\
**Префикс операций** - Префикс для стандартных операций API Mindbox. Значение нужно уточнить у менеджера.\
**Бренд** - Наименование бренда для подписок. 
#### Идентификаторы
**Идентификатор продукта** - Идентификатор товара, используемый при оформлении заказов. Значение нужно уточнить у менеджера.\
**Идентификатор клиента** - Идентификатор сайта в системе Mindbox. Значение нужно уточнить у менеджера.\
**Идентификатор заказа** - Идентификатор заказов сайта в системе Mindbox, используется при оформлении заказов. Значение нужно уточнить у менеджера.
            ...
            ---- 2019/
                ------ 04/
                    ...
                    -------- 30/
                        ---------- mindbox.log
```
**Клиент HTTP** - Тип HTTP клиента, используемого для запросов к API.
...
#### Настройки каталога
**ИД каталога в Битриксе** - Инфоблоки каталога, используемые при выгрузке.\
**Название yml фида** - Имя файла выгрузки относительно DOCUMENT_ROOT вашего проекта. 
...
2. Интегрируем компоненты ```catalog.tracking``` модуля mindbox. Компоненты и их шаблоны находятся в директории - ```/bitrix/components/mindbox```
